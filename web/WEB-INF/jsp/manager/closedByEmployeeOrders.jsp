...
<fmt:message key="common.page.closeOrder" var="close"/>
<fmt:message key="send.to.work" var="sendToWork"/>
<!doctype html>
<html lang="en">
<head>
        ...
        <div class="container" style="height: 69vh">
            <div class="row">
                <table class="table table-striped" style="height: inherit">
                    ...
                    <tbody>
                    <c:forEach var="key" items="${requestScope.closedOrders.keySet()}">
                        <tr>

                            <td>${key.registrationId}</td>
                            <td>${key.userId}</td>
                            <td>${key.street}</td>
                            <td>${requestScope.closedOrders.get(key).userName}</td>
                            <td>${requestScope.closedOrders.get(key).phone}</td>
                            ...
                            <td>
                                <a href="${pageContext.request.contextPath}/controller?command=close_order&registrationId=${key.registrationId}"
                                   style="color: darkgreen">
                                        ${close}
                                </a>
                            </td>
                            <td>
                                <a href="${pageContext.request.contextPath}/controller?command=sent_to_work&registrationId=${key.registrationId}"
                                   style="color: darkgreen">
                                        ${sendToWork}
                                </a>
                            </td>
